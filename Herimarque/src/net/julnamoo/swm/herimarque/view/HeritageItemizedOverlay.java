...
public class HeritageItemizedOverlay extends BalloonItemizedOverlay {
	...
	protected boolean onBalloonTap(int index, OverlayItem item) 
	{
		...
		dataSource.open();
		List<Heritage> items = dataSource.select("crltsNm", item.getTitle());
		dataSource.close();
		Heritage heritage = items.get(0);
		FragmentTransaction ft = fragmentManager.beginTransaction();
		Fragment f = new DetailFragment(heritage, mContext, targetFragmentContainer);
